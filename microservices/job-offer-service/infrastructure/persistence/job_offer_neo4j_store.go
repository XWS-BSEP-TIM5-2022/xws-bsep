...
func (store *JobOfferDBStore) GetRecommendations(user *domain.User, jobOffers []*domain.Post) ([]*domain.PostsID, error) {
	...
	result, err := session.WriteTransaction(func(transaction neo4j.Transaction) (interface{}, error) {
		...
		for _, s := range user.Skills {
			...
			if !checkIfRelationshipExist(user.Id.Hex(), s.Name, transaction) {
				result, err := transaction.Run(
					"MATCH (u:USER) WHERE u.userID=$uIDa "+
						"MATCH (s:SKILL) WHERE s.name=$name "+
						"CREATE (u)-[r:KNOWS]->(s) "+
						"RETURN u.userID",
					map[string]interface{}{"uIDa": user.Id.Hex(), "name": s.Name})
				if err != nil {
					return nil, err
				}
		...
		for _, s := range user.Experience {
			...
			//ako korisnik nije povezan sa vjestinama, dodaje ih
			if !checkIfExpRelationshipExist(user.Id.Hex(), s.Headline, transaction) {
				result, err := transaction.Run(
					"MATCH (u:USER) WHERE u.userID=$uIDa "+
						"MATCH (s:POSITION) WHERE s.headLine=$headLine "+
						"CREATE (u)-[r:WORKED]->(s) "+
						"RETURN u.userID",
					map[string]interface{}{"uIDa": user.Id.Hex(), "headLine": s.Headline})
				if err != nil {
					return nil, err
				}
		...
		for _, job := range jobOffers {
			...
			if !jobOfferExist(job.Id.Hex(), transaction) {
				_, err := transaction.Run(
					...
					map[string]interface{}{"jobID": job.Id.Hex(), "Text": job.Text, "preconditions": job.JobOffer.Preconditions, "position": job.JobOffer.Position.Name})
			...
			if !checkIfJobRelationshipExist(job.Id.Hex(), job.JobOffer.Preconditions, transaction) {
				result, err := transaction.Run(
					"MATCH (j:JOB) WHERE j.jobID=$jobID "+
						"MATCH (s:SKILL) WHERE s.name=$name "+
						"CREATE (j)-[r:NEEDS]->(s) "+
						"RETURN j.jobID",
					map[string]interface{}{"jobID": job.Id.Hex(), "name": job.JobOffer.Preconditions})
				if err != nil {
					return nil, err
				}
			...
			if !checkIfJobPositionRelationshipExist(job.Id.Hex(), job.JobOffer.Position.Name, transaction) {
				result, err := transaction.Run(
					"MATCH (j:JOB) WHERE j.jobID=$jobID "+
						"MATCH (s:POSITION) WHERE s.headLine=$position "+
						"CREATE (j)-[r:INCLUDES]->(s) "+
						"RETURN j.jobID",
					map[string]interface{}{"jobID": job.Id.Hex(), "position": job.JobOffer.Position.Name})
				if err != nil {
					return nil, err
				}
		...
		jobsRecommendations, err1 := getJobRecommendations(user.Id.Hex(), transaction)
		if err1 != nil {
