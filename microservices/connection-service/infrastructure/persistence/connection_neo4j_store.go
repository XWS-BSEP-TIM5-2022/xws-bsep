...
func (store *ConnectionDBStore) ChangePrivacy(ctx context.Context, userIDa string, isPrivate bool) (*pb.ActionResult, error) {
	...
	result, err := session.WriteTransaction(func(transaction neo4j.Transaction) (interface{}, error) {
		...
		if checkIfUserExist(userIDa, transaction) {
			...
			_, err := transaction.Run(
				"MATCH (n:USER) WHERE n.userID = $u1ID set n.isPublic = $isPrivate",
				map[string]interface{}{"u1ID": userIDa, "isPrivate": isPrivate})

			if err != nil {
				actionResult.Msg = "Error while changing privacy of user with ID:" + userIDa
				return actionResult, err
			}
		...
		actionResult.Msg = "Successfully changed privacy of user with ID:" + userIDa
